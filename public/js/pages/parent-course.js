// public/js/pages/parent-course.js
...
(async function () {
  ...
  if (!childId || !courseId) { window.location.href = '/parent/courses.html'; return; }

  const content = document.getElementById('content');

  try {
    const { course, lessons, child } = await api.get(`/api/parent/children/${childId}/courses/${courseId}`);
    document.getElementById('page-title').textContent = `${course.title} · Классный журнал`;
    render(course, lessons, child);
  } catch (err) {
    content.innerHTML = `<div class="empty-state"><p class="empty-state__text">${escapeHtml(err.message)}</p></div>`;
  }

  function render(course, lessons, child) {
    ...
    content.innerHTML = `
      <div class="page-head">
        <div>
          <p class="page-head__eyebrow">Курс · ${escapeHtml(child.firstName)} ${escapeHtml(child.lastName)}${course.academicYear ? ` · ${escapeHtml(course.academicYear)}` : ''}</p>
  ...
  function lessonCardHtml(lesson) {
    ...
    return `
      <a class="item-card" href="/parent/lesson.html?childId=${encodeURIComponent(childId)}&id=${encodeURIComponent(lesson.id)}">
